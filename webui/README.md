...
The engine serves a native React management editor at `/management.html`. OpenAI compatibility, Codex and Claude provider forms contain **Advanced settings → MonkeyCode · signing_secret**. The field uses the existing provider save operation; clearing it disables signing. It requires one paired API key and an explicit Base URL. Codex WebSockets are disabled while signing is enabled.
